...
async def warn(ctx,member:discord.Member,*,reason="No Reason Provided"):
  ...
  embed.add_field(name="Action: Warn",value=f'By <@{ctx.message.author.id}>',inline=True)
...
async def serverinfo(ctx):
  ...
  em.add_field(name='Text Channels', value=f'```{str(channel_count)}```',inline=False)
  ...
  em.add_field(name='Created At', value=f"{server.created_at.__format__('%A, %d. %B %Y')}",inline=False)
